...
const NEWTV_FONTS =
  'https://fonts.googleapis.com/css2?family=Orbitron:wght@500;800&' +
  'family=Barlow+Condensed:wght@500;700&family=VT323&display=swap';
...
const NEWTV_CSS = String.raw`
  :host{
    --void:#05060A; --panel:#0C0E16; --line:#1B1F2E;
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  #portal .brand{font-family:'Orbitron',sans-serif;font-weight:800;font-size:clamp(30px,6vw,58px);letter-spacing:8px;
    background:linear-gradient(180deg,#fff,#7d8496);-webkit-background-clip:text;background-clip:text;color:transparent;}
  #portal .tag{font-family:'VT323',monospace;font-size:19px;color:var(--dim);letter-spacing:4px;margin-top:-16px;}
  ...
  #hero .live{font-family:'Orbitron',sans-serif;font-size:12px;letter-spacing:3px;color:#fff;background:var(--hero-tint);padding:4px 10px;border-radius:6px;white-space:nowrap;animation:pulse 2s infinite;}
  @keyframes pulse{50%{opacity:.55;}}
  @media (prefers-reduced-motion:reduce){#hero .live{animation:none;}}
  #hero h2{font-family:'Orbitron',sans-serif;font-size:clamp(17px,3vw,24px);letter-spacing:2px;color:var(--hero-tint);}
  ...
  .megabtn h2{font-family:'Orbitron',sans-serif;font-size:clamp(20px,3.2vw,30px);letter-spacing:4px;color:var(--glow);}
  ...
  .tile h3{font-family:'Orbitron',sans-serif;font-size:16px;letter-spacing:2px;color:var(--tint);}
  .tile p{font-size:13px;color:var(--dim);margin-top:5px;line-height:1.25;}
  #back{display:none;font-family:'VT323',monospace;font-size:20px;color:var(--dim);background:none;border:1px solid var(--line);
  ...
  /* ================= TV ENGINE ================= */
  #tv{position:relative;height:100%;display:none;flex-direction:column;background:var(--void);}
  ...
  #menuchip{position:absolute;top:10px;left:12px;z-index:8;background:rgba(0,0,0,.65);color:var(--accent);border:1px solid var(--accent);font-family:'Orbitron',sans-serif;font-size:12px;letter-spacing:2px;padding:6px 12px;border-radius:8px;cursor:pointer;}
  #menuchip:hover{background:var(--accent);color:#000;}
  #osd{position:absolute;top:12px;left:52px;top:52px;left:16px;font-family:'Orbitron',sans-serif;font-weight:800;font-size:26px;color:var(--accent);text-shadow:0 0 12px var(--accent),1px 1px 0 #000;letter-spacing:3px;pointer-events:none;z-index:7;opacity:0;transition:opacity .3s;}
  #osd.show{opacity:1;}
  #clock{position:absolute;top:12px;right:16px;font-family:'VT323',monospace;font-size:30px;color:var(--accent);text-shadow:1px 1px 0 #000;pointer-events:none;z-index:7;}
  #nowbar{position:absolute;left:0;right:0;bottom:0;background:linear-gradient(0deg,rgba(0,0,0,.9),transparent);padding:26px 16px 8px;font-family:'VT323',monospace;font-size:20px;color:var(--text);text-shadow:1px 1px 0 #000;pointer-events:none;z-index:7;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
  ...
  #radiocard .onair{font-family:'Orbitron',sans-serif;font-size:26px;letter-spacing:6px;color:var(--accent);}
  #radiocard .rtitle{font-family:'VT323',monospace;font-size:19px;color:var(--dim);margin-top:10px;max-width:60vw;}
  ...
  #ticker .reel{display:inline-block;white-space:nowrap;font-family:'VT323',monospace;font-size:21px;color:var(--accent);padding-left:100%;animation:crawl 30s linear infinite;}
  ...
  #timebar div{font-family:'Orbitron',sans-serif;font-weight:500;font-size:14px;color:var(--accent);letter-spacing:3px;padding-left:10px;text-transform:uppercase;}
  #timebar div:first-child{font-family:'VT323',monospace;font-size:18px;color:var(--dim);letter-spacing:1px;}
  ...
  .ch b{font-family:'Orbitron',sans-serif;font-size:15px;line-height:1.15;color:var(--text);}
  ...
  .slot em{font-style:normal;font-family:'VT323',monospace;font-size:14px;color:var(--accent);}
  ...
  #controls button{flex:1;background:#000;color:var(--accent);border:0;border-right:1px solid var(--line);font-family:'Orbitron',sans-serif;font-size:13px;letter-spacing:2px;cursor:pointer;}
  ...
  #stats{display:none;position:absolute;inset:6% 8%;z-index:95;background:#000;border:2px solid var(--accent);color:var(--text);font-family:'VT323',monospace;padding:18px;overflow:auto;}
  #stats h3{color:var(--accent);letter-spacing:3px;margin-bottom:10px;font-size:24px;font-family:'Orbitron',sans-serif;}
    ...
    const onKey = (handler) => {
      document.addEventListener('keydown', (e) => {
        if (!host.contains(document.activeElement) &&
            root.activeElement === null) return;
    ...
    try {
      {
        ...
        west:{label:"WESTERNS TV",accent:"#E07B39",junk:/\b(trailer|slideshow|review|reaction)\b/i,maxLen:8000,tick:"SADDLE UP \u2022 LONE RANGER \u00b7 CISCO KID \u00b7 ANNIE OAKLEY \u00b7 WESTERN CINEMA",blurb:"Lone Ranger to B-westerns.",channels:[
          ...
          {num:5,call:"BNZA",name:"BONANZA",type:"query",est:3000,q:'title:(bonanza) AND mediatype:(movies) AND year:[1959 TO 1964]'},
        ...
        comedy:{label:"COMEDY TV",accent:"#FFB020",junk:/\b(trailer|review|reaction)\b/i,maxLen:8000,tick:"LAUGH TRACK INCLUDED \u2022 HILLBILLIES \u00b7 JACK BENNY \u00b7 GROUCHO \u00b7 MAYBERRY",blurb:"Hillbillies, Jack Benny, Groucho, Mayberry.",channels:[
          {num:2,call:"HILL",name:"BEVERLY HILLBILLIES",type:"query",est:1600,q:'title:("beverly hillbillies") AND mediatype:(movies) AND year:[1962 TO 1964]'},
          ...
          {num:8,call:"MAYB",name:"MAYBERRY",type:"query",est:1600,q:'title:("andy griffith") AND mediatype:(movies) AND year:[1962 TO 1963]'},
        ...
        function daypart(){
          ...
          return {net:"midnight",label:"INSOMNIA THEATER",blurb:"You're up anyway. Don't watch alone."};
        }
        ...
        const subnetsEl=$id('subnets'), toprow=$id('toprow'),
              backBtn=$id('back'), heroEl=$id('hero');
        function paintHero(){
          const dp=daypart(), n=NETWORKS[dp.net];
          heroEl.style.setProperty('--hero-tint',n.accent);
          heroEl.innerHTML='<span class="live">ON RIGHT NOW</span><span><h2>'+dp.label+'</h2><p>'+dp.blurb+' \u2192 '+n.label+'</p></span>';
          heroEl.onclick=()=>enterNetwork(dp.net);
        }
        ...
        function enterNetwork(key){
          ...
          const tick=NET.tick+'  \u2022  FREE TV. FOREVER.  \u2022  '+sponsorLine()+'  \u2022  ';
            ...
            if(net.adBrandsAll){
              Object.keys(groups).sort().slice(0,MAX_BRAND_CHANNELS).forEach(b=>{
                num++; chans.push({num,call:b.replace(/[^a-z0-9]/gi,'').slice(0,4).toUpperCase(),
                  name:b.toUpperCase(),type:'query',est:90,q:'collection:('+groups[b].join(' OR ')+')'});
              });
            }else{
        ...
        function exitToMenu(){
          ...
          subnetsEl.style.display='none'; backBtn.style.display='none'; toprow.style.display='flex'; heroEl.style.display='flex';
          paintHero(); current=null; NET=null;
        }
...
async function fetchDocs(q){
        ...
        async function fetchItemFiles(itemId){
          ...
          const out=Object.values(byBase).slice(0,250).map(({f})=>({
            ...
            len:Math.max(5,parseLen(f.length)||600)
          }));
          ...
          try{
            if(ch.type==='items'){
              ...
              ch.queue=docs.filter(d=>{
                ...
                if(y&&!t.includes(String(y)))t+=' ('+y+')';
                return {id:d.identifier,title:t,len:ch.est||600};
              });
        ...
        function livePosQuery(ch){
          return {index:Math.floor(Date.now()/1000/(ch.est||600))%ch.queue.length,seek:0};
        }
        function upcoming(ch,count){
          ...
          const out=[{title:list[i].title,startsIn:0}]; let acc=until;
          for(let k=1;k<count;k++){ i=(i+1)%list.length;
            out.push({title:list[i].title,startsIn:acc}); acc+=list[i].len; }
          return out;
        }
            ...
            if(ch.failed) cells='<div class="slot"><i>OFF AIR</i></div><div class="slot"></div><div class="slot"></div>';
              ...
              try{
                cells=upcoming(ch,3).map((u,k)=>{
                  const label=k===0?(current===ch.num?'\u25B6 ON NOW':'NOW'):fmt(new Date(Date.now()+u.startsIn*1000));
        ...
        function step(dir){ const nums=CHANNELS.map(c=>c.num);
          if(current===null){tune(nums[0]);return;}
          tune(nums[(nums.indexOf(current)+dir+nums.length)%nums.length]); }
        
        if(IS_TOUCH){ vid.muted=true; $id('btnMute').textContent='SOUND: OFF'; }
        ...
        function toggleStats(){
          if(!NET)return;
          if(statsEl.style.display==='block'){statsEl.style.display='none';return;}
          let totItems=0,totSec=0,body='';
          CHANNELS.forEach(ch=>{
            const list=ch.playlist||ch.queue||[];
            const n=list.length, sec=list.reduce((s,p)=>s+(p.len||0),0);
            totItems+=n; totSec+=sec;
            body+='<tr><td>'+String(ch.num).padStart(2,'0')+' '+ch.name+'</td><td>'
              +(ch.failed?'OFF AIR':(n||'\u2026'))+'</td><td>'+(n?(sec>=3600?(sec/3600).toFixed(1)+' hr':Math.round(sec/60)+' min'):'\u2014')+'</td></tr>';
          });
          statsEl.innerHTML='<h3>'+NET.label+' INVENTORY</h3><table><tr><td>CHANNEL</td><td>ITEMS</td><td>RUNTIME</td></tr>'
            +body+'<tr class="tot"><td>TOTAL ON FILE</td><td>'+totItems+'</td><td>'+(totSec/3600).toFixed(1)+' hrs</td></tr></table>'
            +'<p>Runtimes are estimates until each item has played once. Press S to close.</p>';
